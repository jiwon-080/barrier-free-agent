...
└── financial_advisor_agent (서브에이전트)
    ...
    ├── search_products / get_product_detail / compare_products  # 예·적금 상품 조회
    ├── get_etf_price / get_etf_prices_by_keyword  # ETF 시세
    └── get_macro_indicators      # 거시경제 지표 (기준금리·환율·물가)
```
...
│   ├── product_tool.py         # 예·적금·ISA·IRP 상품 조회
│   ├── krx_tool.py             # ETF 시세
│   ├── macro_tool.py           # 거시경제 지표
...
│
├── data/
│   ├── rag/                    # 금융 용어사전, 상품 데이터, GraphRAG pkl
│   └── tos/                    # 약관 텍스트
│
├── scripts/
