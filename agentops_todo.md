...
## Phase 1. 로컬 테스트 환경 구축

- [x] **Agent Starter Pack으로 프로젝트 부트스트래핑**
  - 로컬 구동을 위한 `app/`, `tests/` 디렉토리 생성 및 `uv` 환경 세팅 완료
  - 평가 데이터셋 디렉토리 뼈대 생성 (`data/eval/`)

- [x] **로컬 테스트(Layer 1, 2) 파이프라인 확립**
  - `pytest` 및 비동기 테스트 패키지 설치 완료
  - 도구(Tool) 검증용 단위 테스트 뼈대 작성 (`tests/unit/test_dummy.py` 통과)
  - **작업 규칙:** 앞으로 도구 코드를 수정할 때마다 터미널에서 수동으로 `pytest tests/unit/`을 실행하여 버그를 차단함.
